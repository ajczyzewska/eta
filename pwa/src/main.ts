import { Player, type Book, type Manifest, type PlayerState } from './player';
...
const MANIFEST_URL = import.meta.env.DEV
  ? `${import.meta.env.BASE_URL as string}audiobooks/manifest.json`
...
function initUI(manifest: Manifest): void {
  ...
  const progressSeek = document.getElementById('progress-seek') as HTMLInputElement;
  ...
  let lastBookSlug: string | null = null;
  ...
  function renderBooks(): void {
    ...
    books.forEach(book => {
      ...
      if (selectedBook?.slug === book.slug) li.classList.add('active');
      li.addEventListener('click', () => { selectBook(book); });
      booksList.appendChild(li);
    });
  }

  function renderChapters(activeIndex: number): void {
    if (!selectedBook) { chaptersList.innerHTML = ''; return; }
  ...
  function updateChapterHighlight(index: number): void {
    const items = chaptersList.querySelectorAll('li');
    items.forEach((li, i) => { li.classList.toggle('active', i === index); });
    scrollActiveChapter();
  }

  function scrollActiveChapter(): void {
    (chaptersList.querySelector('.active') as HTMLElement | null)
      ?.scrollIntoView({ block: 'nearest' });
  }

  // ── Book selection ────────────────────────────────────────────

  function selectBook(book: Book, chapterIndex = 0, resumeTime = 0): void {
    selectedBook = book;
  ...
  player.onStateChange = (state: PlayerState) => {
    if (state.book) {
      ...
      if (chapterChanged) {
        ...
        if (chapter) updateMediaMetadata(state.book, chapter);
      }
    } else {
    ...
    btnPlayPause.textContent = state.isPlaying ? '⏸ PAUZA' : '▶ ODTWÓRZ';
    if (state.error) {
  ...
  if (resume) {
    ...
    if (book) {
      ...
      const safeIndex =
        resume.chapterIndex < book.chapters.length ? resume.chapterIndex : 0;
      const safeTime =
        safeIndex === resume.chapterIndex && resume.currentTime >= 0
          ? resume.currentTime
          : 0;
